...
function gainAbsurdity(n) {
  game.absurdity += n;
  game.totalAbsurdityEarned += n;
}

function hasPerk(id) {
  return game.purchasedPerks.has(id);
}
...
// Based on LIFETIME absurdity: spending the balance on perks costs nothing.
function absurdityFactor() {
  return Math.pow(1 + game.totalAbsurdityEarned, 0.19);
}
